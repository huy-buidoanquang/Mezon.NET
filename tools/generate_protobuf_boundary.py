...
def gen_realtime_iface_method(method: RealtimeMethod) -> str:
    if method.response_data_type:
        ret = f"Task<{facade_type_ref(method.response_data_type)}>"
    else:
        ret = "Task"
    if method.no_params:
...
def gen_realtime_method_impl(method: RealtimeMethod) -> str:
    ...
    if method.response_data_type:
        ...
        {{
            {envelope_line}
            var response = await SendRtAwaitAckAsync(envelope, options).ConfigureAwait(false);
    ...
    return f"""        /// <summary>Realtime envelope send ({method.envelope_property}). mezon-js: {method.js_name}.</summary>
        ...
        {{
            {envelope_line}
            await SendRtAsync(envelope, options).ConfigureAwait(false);
        }}"""
