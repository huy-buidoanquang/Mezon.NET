#!/usr/bin/env python3
"""Generate protobuf boundary layer: *Params, *Data view structs, and API facades."""
...
EVENTS_FILE = CLIENT / "BaseSocketClient.Events.cs"
...
REALTIME_METHODS: List[RealtimeMethod] = [
    ...
    RealtimeMethod(
        ...
        response_data_type="ChannelMessageAckData",
    ),
...
def gen_data_struct(msg: ProtoMessage, api_msgs: Dict[str, ProtoMessage], rt_msgs: Dict[str, ProtoMessage]) -> str:
    type_name = data_type_name(msg.name)
    ...
    for fld in msg.fields:
        ...
        if fld.repeated:
            ...
            if inner_name in api_msgs or inner_name in rt_msgs:
                inner_data = data_type_name(inner_name)
                lines.append(
...
def gen_event_data_struct(msg: ProtoMessage, api_msgs: Dict[str, ProtoMessage], rt_msgs: Dict[str, ProtoMessage]) -> str:
    ...
    data_name = data_type_name(msg.name)
    return f"""// <auto-generated />
#nullable enable
...
def gen_method_impl(method: ApiMethod, *, is_socket: bool, api_msgs: Dict[str, ProtoMessage], rt_msgs: Dict[str, ProtoMessage]) -> str:
    ...
    if wrap:
        return f"""        public async Task<{facade_ret}> {method.name}({sig_params})
        {{
            var result = await ApiClient.{method.name}({call_args}).ConfigureAwait(false);
            return new {facade_ret}(result);
        }}"""
...
def gen_iface_method(method: ApiMethod, api_msgs: Dict[str, ProtoMessage], rt_msgs: Dict[str, ProtoMessage]) -> str:
    ...
    return f"        Task<{facade_ret}> {method.name}({sig_params});"
...
# Fallback when BaseSocketClient.Events.cs no longer lists protobuf payload types.
SOCKET_EVENT_MANIFEST: List[Tuple[str, str, str]] = [
    ...
    ("UserChannelAddedEvent", "UserChannelAdded", API_NS),
...
def gen_realtime_iface_method(method: RealtimeMethod) -> str:
    if method.response_data_type:
        ret = f"Task<{method.response_data_type}>"
    else:
        ret = "Task"
    if method.no_params:
        return f"        {ret} {method.dotnet_name}(RequestOptions? options = null);"
    params_type = f"{NS_MODELS}.{method.params_type}"
    return f"        {ret} {method.dotnet_name}({params_type} {method.param_name}, RequestOptions? options = null);"
...
def gen_realtime_method_impl(method: RealtimeMethod) -> str:
    ...
    if method.response_data_type:
        ...
        public async Task<{method.response_data_type}> {method.dotnet_name}({sig})
        {{
            {envelope_line}
            var response = await SendRealtimeEnvelopeAsync(envelope, options).ConfigureAwait(false);
            return new {method.response_data_type}(response.{method.response_property});
        }}"""
...
def gen_realtime_interface() -> str:
    lines = [
        "// <auto-generated />",
        "#nullable enable",
        "using System.Threading.Tasks;",
        ...
        "using Mezon.Net.Core;",
        "",
        f"namespace {NS_ABSTRACTIONS}",
        "{",
        "    public interface IMezonClientRealtime",
        "    {",
    ]
    for method in REALTIME_METHODS:
        lines.append(gen_realtime_iface_method(method))
    lines.extend(["    }", "}", ""])
    return "\n".join(lines)
...
def main() -> int:
    ...
    for name in sorted(all_data_types):
        ...
        if not msg:
            continue
        write_file(OUT_RESPONSES / f"{data_type_name(name)}.g.cs", gen_data_struct(msg, api_messages, rt_messages))
        generated += 1
    ...
    for m in methods:
        iface_lines.append(gen_iface_method(m, api_messages, rt_messages))
    iface_lines.extend(["    }", "}", ""])
    write_file(OUT_GENERATED / "IMezonClientApi.g.cs", "\n".join(iface_lines))
    generated += 1
    ...
    for m in socket_api_methods:
        ...
        socket_lines.append("")
    socket_lines.extend(["    }", "}", ""])
    write_file(OUT_GENERATED / "BaseSocketClient.Api.g.cs", "\n".join(socket_lines))
    generated += 1

    write_file(OUT_GENERATED / "BaseSocketClient.Events.Data.g.cs", gen_events_file(events))
    generated += 1

    write_file(OUT_GENERATED / "IMezonClientRealtime.g.cs", gen_realtime_interface())
    generated += 1
    write_file(OUT_GENERATED / "BaseSocketClient.Realtime.g.cs", gen_realtime_facade())
    generated += 1
    ...
    print(f"  Data types: {len(all_data_types)}")
