...
namespace Mezon.NET.Api
{
    ...
    internal struct ClientBucket
    {
        ...
        static ClientBucket()
        {
            var buckets = new[]
            {
                new ClientBucket(ClientBucketType.Unbucketed, BucketId.Create(string.Empty, "<unbucketed>", []), 10, 10),
                new ClientBucket(ClientBucketType.SendEdit, BucketId.Create(string.Empty, "<send_edit>", []), 10, 10)
            };
