                    ...
                    if (response.StatusCode < (HttpStatusCode)200 || response.StatusCode >= (HttpStatusCode)300)
                    {
                        switch (response.StatusCode)
                        {
                            ...
                            case HttpStatusCode.BadGateway: //502
                                ...
                                {
                                    throw new HttpException(HttpStatusCode.BadGateway, request, MezonErrorCode.GeneralError);
                                }
                            ...
                            default:
                                ...
                                throw new HttpException(
                                    ...
                                    error?.Errors.IsSpecified == true ?
                                        error.Errors.Value.Select(x => new MezonJsonError(x.Name.GetValueOrDefault("root"), x.Errors.Value.Select(y => new MezonError(y.Code!, y.Message!)).ToArray())).ToArray() :
                                        null
                                    );
                        }
                    }
        ...
        private async Task EnterAsync(int id, IRequest request)
        {
            ...
            while (true)
            {
                ...
                if (windowCount >= 0 && semaphore < 0)
                {
                    ...
                    {
                        if (timeoutAt.HasValue && ((timeoutAt.Value - DateTimeOffset.UtcNow).TotalMilliseconds < MinimumSleepTimeMs))
                        {
                            ThrowRetryLimit(request);
                        }
#if DEBUG_LIMITS
        ...
        private void UpdateRateLimit(int id, IRequest request, RateLimitInfo info, bool is429, bool redirected = false, Stream? body = null)
        {
            ...
            lock (_lock)
            {
                ...
                if (is429)
                {
                    ...
                    RatelimitResponse? payload = null;
                    if (body != null)
                    {
                        payload = info.ReadRatelimitPayload(body);
                    }
