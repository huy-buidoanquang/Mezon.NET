...
namespace Mezon.Net.Client
{
    public partial class MezonClient
    {
        private async Task ProcessMessageAsync(MezonMessageType type, int cid, int code, ReadOnlyMemory<byte>? data, Envelope? envelope)
        {
            ...
            {
                await _socketLogger.DebugAsync("Received: API").ConfigureAwait(false);
                return;
            }
            else if (type == MezonMessageType.Abridged)
            {
                await _socketLogger.DebugAsync("Received: ABRIDGED").ConfigureAwait(false);
                if (envelope == null)
                {
                    return;
                }
                await _socketLogger.DebugAsync("Envelop:" + envelope?.ToString()).ConfigureAwait(false);
