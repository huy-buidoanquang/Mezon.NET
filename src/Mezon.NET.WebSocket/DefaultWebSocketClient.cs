...
{
    internal class DefaultWebSocketClient : IWebSocketClient, IDisposable, IAsyncDisposable
    {
        ...
        private async Task RunAsync(CancellationToken cancelToken)
        {
            ...
            try
            {
                while (!cancelToken.IsCancellationRequested)
                {
                    ...
                    {
                        throw new WebSocketClosedException(socketResult.CloseStatus ?? WebSocketCloseStatus.Empty, socketResult.CloseStatusDescription);
                    }
                    ...
                    if (!socketResult.EndOfMessage)
                    {
                        ...
                        using (var stream = new MemoryStream())
                        {
                            ...
                            var streamBuffer = new ArraySegment<byte>();
                            if (stream != null)
                            {
                                result = stream.TryGetBuffer(out streamBuffer) ? streamBuffer.Array : stream.ToArray();
                            }
                        }
                    }
                    else
